...
void main() {
  ...
  test('Verify Color Search Logic', () async {
    final repo = MockProductRepository();
    // Assuming mock data has 'Red Apple' and 'Green Spinach'

    final redProducts = await repo.searchProductsByColor('Red');
    expect(redProducts.every((p) => p.name.contains('Red') || p.name.contains('Apple') || p.name.contains('Tomato')), isTrue);
    expect(redProducts.isNotEmpty, isTrue);

    final greenProducts = await repo.searchProductsByColor('Green');
    expect(greenProducts.every((p) => p.name.contains('Green') || p.name.contains('Spinach')), isTrue);
    expect(greenProducts.isNotEmpty, isTrue);
  });
}
