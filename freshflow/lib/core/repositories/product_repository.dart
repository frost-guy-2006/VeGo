...
class ProductRepository {
  ...
  /// Search products by color (server-side inference)
  Future<List<Product>> searchProductsByColor(String color) async {
    // Get keywords for the color from Product model
    final keywords = Product.colorKeywords[color] ?? [];
    if (keywords.isEmpty) return [];

    // Construct OR query for all keywords
    // name.ilike.%keyword1%,name.ilike.%keyword2%
    final orQuery =
