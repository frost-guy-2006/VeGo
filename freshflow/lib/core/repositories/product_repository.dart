...
class ProductRepository {
  ...
  Future<List<Product>> searchProductsByColor(String color) async {
    ...
    } else if (lowerColor == 'orange' || lowerColor == 'yellow') {
      queryStr =
          'name.ilike.%carrot%,name.ilike.%banana%,name.ilike.%orange%,name.ilike.%yellow%';
    } else {
      return searchProducts(color);
    }

    final response =
        await _client.from('products').select().or(queryStr).order('name');
