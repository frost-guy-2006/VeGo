...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _performSearch(String query) async {
    ...
    // Check for color keywords using the single source of truth in Product model
    for (var entry in Product.colorKeywords.entries) {
      if (entry.key.toLowerCase() == lowerQuery ||
          entry.value.contains(lowerQuery)) {
        matchedColor = entry.key;
        break;
      }
    }
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Optimized server-side filtering for colors
        results = await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Optimized server-side filtering for name
        results = await _productRepository.searchProducts(query);
      }

      if (!mounted) return;

      // Ensure the result is still relevant to the current query in the input
      // (Handling race conditions where a previous slow request returns after a new one started)
      // Note: With debounce this is less likely but still possible.
