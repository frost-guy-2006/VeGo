...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    // Check if the query matches exactly a supported color key (case-insensitive)
    final matchedColorKey = Product.colorKeywords.keys.firstWhere(
      (key) => key.toLowerCase() == lowerQuery,
      orElse: () => '',
    );

    if (matchedColorKey.isNotEmpty) {
      _activeColorFilter = matchedColorKey;
    } else {
      _activeColorFilter = null;
    }
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Perform server-side search by color keyword mapping
        results = await _productRepository.searchProductsByColor(
          _activeColorFilter!,
        );
      } else {
        // Perform standard server-side text search
        results = await _productRepository.searchProducts(trimmedQuery);
      }

      if (!mounted) return;

      // If user cleared the input before the query finished
