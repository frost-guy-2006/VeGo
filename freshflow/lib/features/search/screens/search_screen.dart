...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if (widget.initialQuery != null) {
      // Use addPostFrameCallback to ensure widget is mounted before searching
  ...
  Future<void> _performSearch(String query) async {
    // Cancel any existing timer if this method is called directly (though typically handled by debounce)
    _debounceTimer?.cancel();
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Filter by inferred color using optimized server-side query
        filtered = await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Filter by name using optimized server-side query
        filtered = await _productRepository.searchProducts(lowerQuery);
      }

      // Check if text has changed while waiting for async result to avoid race conditions
