...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void initState() {
    ...
    if (widget.initialQuery != null) {
      ...
      // Use addPostFrameCallback to avoid setState during build if initial query triggers immediate update
  ...
  void _performSearch(String query) async {
    ...
    final capitalizedQuery =
        lowerQuery.isNotEmpty ? lowerQuery[0].toUpperCase() + lowerQuery.substring(1) : '';

    if (Product.colorKeywords.containsKey(capitalizedQuery)) {
      _activeColorFilter = capitalizedQuery;
    } else {
      _activeColorFilter = null;
    }
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Filter by inferred color on server
        results = await _productRepository
            .searchProductsByColor(_activeColorFilter!);
      } else {
        // Search by name on server
        results = await _productRepository.searchProducts(query);
      }

      if (mounted) {
        // Check if query has changed while we were awaiting (simple race condition check)
