...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    // Check for color keywords in Product.colorKeywords keys
    final lowerQuery = query.toLowerCase();
    String? matchedColor;

    // Check if the query matches any of the color keys directly (case insensitive)
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Visual Search via Repository
        results =
            await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Standard Text Search via Repository
        results = await _productRepository.searchProducts(query);
      }

      if (!mounted) return;
      // Race condition check: ensure query hasn't changed while awaiting
