  ...
  void _performSearch(String query) async {
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Fetch all products and filter by inferred color client-side
        // since backend doesn't support searching by color yet
        final allProducts = await _productRepository.fetchProducts();
        filtered =
            allProducts.where((p) => p.color == _activeColorFilter).toList();
      } else {
        // Delegate search to the repository (server-side query)
        filtered = await _productRepository.searchProducts(query);
      }

      if (!mounted || _searchController.text.trim().toLowerCase() != query.trim().toLowerCase()) return;
