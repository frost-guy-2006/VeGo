  ...
  Future<void> _performSearch(String query) async {
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Use server-side filter by inferred color
        results =
            await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Use server-side filter by name
        results = await _productRepository.searchProducts(lowerQuery);
      }

      if (!mounted) return;

      // If the user cleared the search field while network request was pending, avoid updating results
