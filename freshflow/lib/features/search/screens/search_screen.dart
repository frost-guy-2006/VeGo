...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _executeSearch(String query) async {
    ...
    String? localActiveColorFilter;
    if (['red', 'blue', 'green', 'orange', 'yellow'].contains(lowerQuery)) {
      localActiveColorFilter = lowerQuery;
      // Capitalize first letter for display
      localActiveColorFilter =
          lowerQuery[0].toUpperCase() + lowerQuery.substring(1);
    } else {
      localActiveColorFilter = null;
    }

    if (mounted) {
      setState(() {
        _activeColorFilter = localActiveColorFilter;
      });
    }

    try {
      List<Product> filtered;
      if (localActiveColorFilter != null) {
        // Filter by server-side color search
        filtered = await _productRepository
            .searchProductsByColor(localActiveColorFilter);
      } else {
        // Filter by server-side name search
        filtered = await _productRepository.searchProducts(lowerQuery);
      }

      // Check if query is still valid (in case another search started, though debounce helps)
