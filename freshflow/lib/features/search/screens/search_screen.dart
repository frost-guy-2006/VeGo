...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    // Check for color keywords exactly (based on memory guidelines for UI visualization fallback)
    final lowerQuery = query.trim().toLowerCase();

    // Determine active color filter by matching exactly against keys in colorKeywords
    String? matchedColor;
    for (final colorName in Product.colorKeywords.keys) {
      if (colorName.toLowerCase() == lowerQuery) {
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        ...
        results = await _productRepository.searchProducts(lowerQuery);
      }

      if (mounted) {
        // Double check that the user hasn't cleared the field while we were searching
