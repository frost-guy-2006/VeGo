...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _performSearch(String query) async {
    ...
    // Check for color keywords using Product.colorKeywords map
    final lowerQuery = query.trim().toLowerCase();
    String? matchedColor;

    // Check if the query matches a color name exactly
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Server-side filtering by color
        filtered = await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Server-side filtering by name
        filtered = await _productRepository.searchProducts(query.trim());
      }

      // Check if user has already cleared search
      if (!mounted) return;
