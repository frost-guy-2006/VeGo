...
class _SearchScreenState extends State<SearchScreen> {
  ...
  void _performSearch(String query) async {
    ...
    // Use keys from Product.colorKeywords (Red, Green, Blue, etc.)
    final matchedColorEntry = Product.colorKeywords.entries.firstWhere(
      (entry) => entry.key.toLowerCase() == lowerQuery,
      orElse: () => const MapEntry('', []),
    );

    if (matchedColorEntry.key.isNotEmpty) {
      _activeColorFilter = matchedColorEntry.key;
    } else {
      _activeColorFilter = null;
    }
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Optimized: Fetch only products matching the inferred color keywords
        results =
            await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Optimized: Use DB text search instead of fetching all
        results = await _productRepository.searchProducts(query);
      }

      if (!mounted) return;
      // Ensure the results match the current input (race condition check)
