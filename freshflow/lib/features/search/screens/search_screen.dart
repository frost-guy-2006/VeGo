...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    // Check for color keywords (using the shared source of truth)
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Use optimized server-side color keyword filter
        results =
            await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Use optimized server-side name search
        results = await _productRepository.searchProducts(trimmedQuery);
      }

      // Check if the query has changed while we were fetching
