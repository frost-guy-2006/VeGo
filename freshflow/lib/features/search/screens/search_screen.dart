...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    // Check for color keywords exactly
    final lowerQuery = query.toLowerCase();

    // Check if query is an exact color keyword mapped in Product
    String? matchedColor;
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        ...
        // Use standard server-side text search
        filtered = await _productRepository.searchProducts(query);
      }

      if (!mounted) return;

      // Prevent stale data if user cleared field while fetching
