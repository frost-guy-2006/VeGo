...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _performSearch(String query) async {
    ...
    // Check for exact color name matches from our keywords map keys
    final lowerQuery = query.trim().toLowerCase();

    // Default to null
    _activeColorFilter = null;

    // Check if query exactly matches a color category
    for (final colorName in Product.colorKeywords.keys) {
      if (colorName.toLowerCase() == lowerQuery) {
        _activeColorFilter = colorName;
        break;
      }
    }
    ...
    try {
      ...
      if (_activeColorFilter != null) {
        // Filter by visual color on the server-side
        filtered =
            await _productRepository.searchProductsByColor(_activeColorFilter!);
      } else {
        // Filter by name on the server-side
        filtered = await _productRepository.searchProducts(query);
      }

      if (!mounted) return;

      // Ensure the text field hasn't been cleared while we were waiting
